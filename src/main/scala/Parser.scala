      ...
      v.indexOf('.', start) match {
        ...
        case dot =>
          v.indexOf('.', dot + 1) match {
            case -1 =>
              DotAST(pos, expr, pos, LiteralAST(v.substring(dot + 1))) //todo: support integers after '.'
            case idx =>
              fields(idx,
                     DotAST(
                       pos,
                       expr,
                       pos,
                       LiteralAST(v.substring(dot + 1, idx)))) //todo: as above
          }
      }
        ...
        case Some(r1) =>
          ...
          macros get name match {
            case None =>
              commands get name match {
                case None => problem(r2, "expected a command or macro")
                case Some(c) if c.arity == 0 =>
                  problem(r2, "expected a command with parameters")
                case Some(c) =>
