...
[![Version](https://img.shields.io/badge/latest_release-v0.4.9-orange.svg)](https://github.com/edadma/backslash/releases/tag/v0.4.9)
...
```bash
echo "testing \join \v \", \"" | java -jar backslash-0.4.9.jar -j "{v: [\"one\", \"two\", \"three\"]}" --
```
...
libraryDependencies += "xyz.hyperreal" %% "backslash" % "0.4.9"
```

### Executable

An executable can be downloaded from [here](https://dl.bintray.com/edadma/generic/backslash-0.4.9.jar). *You do not need* the Scala library for it to work because the JAR already contains all dependencies. You just need Java 8+ installed.

Run it as a normal Java executable JAR with the command `java -jar backslash-0.4.9.jar <template>` in the folder where you downloaded the file, where *template* is the name of the template file to be rendered.
